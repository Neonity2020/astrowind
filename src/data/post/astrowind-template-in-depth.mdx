...
`npm run build` runs Astro's static build. The sitemap integration writes `sitemap-index.xml`, leaving out the taxonomy pages marked `noindex` and any post that opts out; `rss.xml.ts` writes the feed from the same post list the blog uses; the HTML, CSS and JavaScript are minified; the integration updates `robots.txt`. The result in `dist/` is plain files, with no server component and no environment variables required.
